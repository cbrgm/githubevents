...
func (g *EventHandler) handleBeforeAny(ctx context.Context, deliveryID string, eventName string, event any) error {
	...
	g.mu.RLock()
	handlers := g.onBeforeAny[EventAnyAction]
	g.mu.RUnlock()
	return dispatch[any](ctx, deliveryID, eventName, event, handlers)
}
...
func (g *EventHandler) handleAfterAny(ctx context.Context, deliveryID string, eventName string, event any) error {
	...
	g.mu.RLock()
	handlers := g.onAfterAny[EventAnyAction]
	g.mu.RUnlock()
	return dispatch[any](ctx, deliveryID, eventName, event, handlers)
}
...
func (g *EventHandler) handleError(ctx context.Context, deliveryID string, eventName string, event any, srcErr error) error {
	...
	g.mu.RLock()
	handlers := g.onErrorAny[EventAnyAction]
	g.mu.RUnlock()
	if len(handlers) == 0 {
		return srcErr
	}
	...
	for _, h := range handlers {
		eg.Go(func() (err error) {
			...
			return h(ctx, deliveryID, eventName, event, srcErr)
		})
	}
	return eg.Wait()
}
...
func (g *EventHandler) HandleEventRequest(req *http.Request) error {
	...
	if err != nil {
		return fmt.Errorf("could not validate webhook payload: %w", err)
	}
